function getCookie(name) {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      // Tutor-specific endpoint to fetch request details
  ...
  async function handleRequestAction(action, requestId, token) {
    try {
      // Tutor-specific endpoint to accept/reject join requests
      ...
      if (res.ok) {
        ...
        window.location.href = '/tutoruser/tutordashboard';  // Redirect back to tutor dashboard
      } else {
